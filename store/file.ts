...
export default class LogStore extends VuexModule {
    ...
    async create(): Promise<File | null> {
        try {
            ...
            const json = '{"id":"vimu@0.1.0","nodes":{"1":{"id":1,"data":{},"inputs":{"in_0":{"connections":[]}},"outputs":{},"position":[0,0],"name":"output"}}}'
    ...
    async update(data: { id: string, json?: string, name?: string, favorite?: boolean }): Promise<File | null> {
        try {
            const file: File = await $pb.collection('files').update(data.id, { 'name': data.name, 'json': data.json, 'favorite': data.favorite })
            return file;
        } catch (error) {
